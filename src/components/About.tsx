  ...
  return (
    <section className="py-8 xs:py-12 sm:py-16" id="about">
      <div className="max-w-5xl mx-auto px-3 xs:px-4 sm:px-6">
        <Reveal>
          <div className="relative overflow-hidden rounded-xl xs:rounded-2xl bg-white shadow-lg border border-gray-200">
            <AccentBar />

            <div className="relative p-4 xs:p-5 sm:p-8 grid gap-6 xs:gap-8 sm:gap-10 md:gap-8 md:grid-cols-5 items-start">
              <div className="md:col-span-3">
                <h2 className="mt-1 xs:mt-2 sm:mt-3 text-2xl xs:text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight">
                  About Me
                </h2>
                <div className="mt-4 xs:mt-5 space-y-3 xs:space-y-4 text-gray-700 text-sm xs:text-[15px] sm:text-base leading-relaxed">
                  <p>
              ...
              <div className="md:col-span-2 flex flex-col gap-3 xs:gap-4 max-w-xs xs:max-w-sm w-full mx-auto md:mx-0">
                <div className="relative rounded-xl xs:rounded-2xl overflow-hidden aspect-[4/3] sm:aspect-[5/4] md:aspect-square bg-gray-900 flex items-end justify-start group border border-gray-200 shadow-md ring-1 ring-black/5">
                  <AccentBar />
                  {track?.image ? (
                    <img
                      src={track?.image}
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 gap-2 xs:gap-3">
                  ...
                  <div className="relative z-10 w-full p-2 xs:p-3 sm:p-4 flex flex-col">
                    <div className="inline-block rounded-lg xs:rounded-xl bg-black/55 backdrop-blur-sm px-2 xs:px-3 sm:px-4 py-1.5 xs:py-2 shadow-lg ring-1 ring-white/10 max-w-full">
                      {track ? (
                        ...
                        <span className="text-xs xs:text-sm text-gray-200">
                          Loading track…
                        </span>
                      )}
                    </div>
                    {track && (
                      <p className="mt-1.5 xs:mt-2 sm:mt-3 text-[8px] xs:text-[10px] sm:text-xs uppercase tracking-wide text-gray-300 font-medium">
                        {track.nowPlaying
                          ? "Now Listening to:"
                          : "Last Listened to:"}
                      </p>
